...
매주/격주/매달 회차 날짜는 항상 **시작 날짜를 기준점으로 간격만큼(매주 7일·격주 14일) 앞으로
진행**하며 생성됨 — 입력한 종료 날짜에 맞춰 간격을 조정하거나 역산하지 않음. 그래서 종료 날짜가
그 간격에 정확히 맞아떨어지지 않으면(예: 격주 시작일 기준으로 종료 날짜가 짝이 안 맞는 날),
종료 날짜 자체는 회차로 만들어지지 않고 그 이전의 마지막 정상 회차에서 시리즈가 끝남(의도된
동작 — 종료 날짜는 "이 날짜까지" 상한일 뿐 반드시 마지막 회차가 되는 건 아님). 저장 후에야
알아채고 "회차가 사라졌다"고 오해하지 않도록, 신청 폼(`checkAvail()`)과 관리자 "+ 예약 추가"
모달(`checkEditAvail()`)의 정기 예약 미리보기에 실제 마지막 회차 날짜를 미리 안내하는 문구
(`recurMismatchNote`)를 표시함.
