...
일요일이고 그날 예약이 여러 건이면 "주일예배 (N건)" 칩 하나로 축약함. 클릭하면 체크리스트가 아니라
연결된 예약 상세(openDetail)가 열림 — 자동 생성 체크리스트의 `reservationId`를 우선 쓰고, 없는
레거시 문서는 같은 실·날짜·시작시간의 확정 예약을 찾아 대체 연결함(그래도 못 찾으면 체크리스트로
대체 이동).
