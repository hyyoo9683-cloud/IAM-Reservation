...
| `venue-spaces.html` | 외부 대관 가능 공간 소개 페이지 (로그인 불필요, 공개) — `spaces` 컬렉션을 실시간으로 불러와 용도별(01 세미나실: 아론홀·샤론홀 / 02 소그룹실: 5층, 정원 기준 소형 10~16인·중형 20인 이상 소제목 분리 / 그 외는 "기타 공간")로 그룹핑해 사진·정원·설명·비품 태그 표시, guide.html과 동일한 디자인 톤. 아이엠홀(별도 사이트 예정)·지혜홀(숙박 전용)·이름에 "로비"가 들어간 공간은 목록에서 제외. 공간별 `desc`(짧은 요약, 다른 화면에서도 공용)와 별개로 이 페이지 전용 긴 소개 문구 `venueDesc` 필드가 있으면 그것을 우선 표시 — 두 필드 모두 관리자 "공간 관리" 페이지에서 편집(새 공간 추가도 동일 화면, 이름만 겹치지 않으면 이 페이지에 바로 반영됨) |
