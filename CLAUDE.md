...
일주일 캘린더처럼 오늘~이번 주 일요일까지 요일별 칸으로 나눠 행사 제목만 칩으로 보여주고(아이엠홀
→ 아론홀 → 샤론홀 → 그 외 순 정렬, 아이엠홀만 보라색 칩), 클릭하면 체크리스트가 아니라 연결된
예약 상세(openDetail)가 바로 열림(자동 생성 체크리스트의 `reservationId` 활용, 없으면 체크리스트로
대체 이동).
